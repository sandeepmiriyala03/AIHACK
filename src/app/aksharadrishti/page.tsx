...
async function downloadAsImage(text: string, fontFamily: string, fontSize: number, langLabel: string) {
  const PADDING = 40;
...
export default function AksharaDrishtiPage() {
  ...
  // ✅ FIX: moved from module-level to useRef
  const workerCacheRef = useRef(new Map<string, unknown>());
  ...
  const runOCR = async () => {
    ...
    try {
      ...
      // ✅ FIX: use workerCacheRef.current instead of _workerCache
      let worker = workerCacheRef.current.get(langCombo) as Awaited<ReturnType<typeof Tesseract.createWorker>> | undefined;
      if (!worker) {
        ...
        // ✅ FIX: use workerCacheRef.current
        workerCacheRef.current.set(langCombo, worker);
      }
        ...
        <style>{`
          ...
          *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
          :root {
            ...
            --accent: #1a56e8; --accent2: #6d28d9;
            --success: #059669; --danger: #dc2626;
            --text: #1a1a1a; --text2: #444340; --muted: #7a7870;
            --radius: 14px; --shadow: 0 4px 24px rgba(0,0,0,0.08);
          }
          body { background: var(--bg); color: var(--text); font-family: 'DM Sans', sans-serif; }
          .ad-wrap { min-height:100vh; background:var(--bg); background-image:radial-gradient(ellipse at 10% 0%,rgba(26,86,232,0.05) 0%,transparent 55%),radial-gradient(ellipse at 90% 100%,rgba(109,40,217,0.04) 0%,transparent 55%); display:flex; flex-direction:column; align-items:center; padding:48px 16px 100px; }
          ...
          .ad-combo { margin-top:14px; display:inline-flex; align-items:center; gap:8px; padding:6px 16px; background:var(--surface); border:1px solid var(--border2); border-radius:99px; font-size:13px; color:var(--text2); font-weight:500; box-shadow:var(--shadow); }
          ...
          .ad-tips { width:100%; max-width:920px; margin-bottom:20px; background:var(--surface); border:1px solid var(--border); border-radius:var(--radius); box-shadow:var(--shadow); overflow:hidden; animation:fadeUp 0.5s ease 0.05s both; }
          ...
          .ad-result-area { width:100%; min-height:220px; background:var(--surface2); border:1px solid var(--border2); border-radius:10px; padding:16px; color:var(--text); line-height:1.8; resize:vertical; outline:none; white-space:pre-wrap; word-break:break-word; }
                ...
                {result ? (
                  <>
                    <textarea
                      ...
                      spellCheck={false}
                      style={{fontSize:`${fontSize}px`, fontFamily:`"${activeFont.family}", "Noto Sans", sans-serif`}}
                    />
                    ...
                    <div className="ad-actions">
                      <button className="ad-btn ad-btn-sm ad-btn-blue" onClick={copyText}>{copied?"✓ Copied!":"⎘ Copy text"}</button>
                      <button className="ad-btn ad-btn-sm ad-btn-green" onClick={handleDownload}>↓ Download PNG</button>
